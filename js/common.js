...
function popOpen(padding, id) {
	// 팝업 열기 onclick
	// scrollFixed();
	// dim
	$("#menu-overlay").addClass("pop-on").attr("data-pop-id", id).fadeIn(300);

	$("#" + id + "")
		.css({ padding: "" + padding + "", "margin-top": -$("#" + id + "").outerHeight() / 2 })
		.addClass("pop-on")
		.fadeIn(300);
}
...
function dataPicker(data) {
	...
	el.datepicker({
		...
		monthNames: [
			"<strong>1</strong> January",
			"<strong>2</strong> Fedruary",
			"<strong>3</strong> March",
			"<strong>4</strong> April",
			"<strong>5</strong> May",
			"<strong>6</strong> June",
			"<strong>7</strong> July",
			"<strong>8</strong> August",
			"<strong>9</strong> September",
			"<strong>10</strong> October",
			"<strong>11</strong> November",
...
function galleryPOP(toggle, type, index) {
	// 갤러리팝업 열고,닫기
...
function galleryType2(toggle, index) {
	// 사각형 갤러리
	var el = $(".gallery-pop-wrap");
